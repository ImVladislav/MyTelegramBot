...
bot.onText(
  /\/start/,
  (msg: { chat: { id: any }, from: { first_name: any }, text: any }) => {
    ...
    bot.sendMessage(
      chatId,
      `Вітаю, ${userName}! Мене звати Альберт Авксентійович, я бот-асистент Владислава! Сподіваюся зможу вам надати інформацію, яка вас цікавить.`,
      {
        reply_markup: {
          keyboard: [
...
bot.onText(/Надай посилання на його роботи/, (msg: { chat: { id: any } }) => {
  ...
  bot.sendMessage(
    chatId,
    `Звісно, на мою думку, це найкращий його проект, але ще є багато інших проектів. Який саме вас цікавить?`,
    { reply_markup: keyboardWithProjects }
  );
});
...
bot.onText(/Надай мені Резюме Владислава/, (msg: { chat: { id: any } }) => {
  ...
  bot
    .sendDocument(chatId, resumePath)
    .then(() => {
...
bot.onText(/Інтернет магазин Beautyblossom/, (msg: { chat: { id: any } }) => {
  ...
  bot.sendMessage(
    chatId,
    `Beauty blossom це потужний інтернет магазин, в якому є великий вибір товарів для краси, здоровя та дому. Владислав на цьому пректі героїчно взяв на себе роль Full-stack розробника, та майже за місяць 2 розробники написали цей чудовий сайт!`
  );

  setTimeout(() => {
    bot.sendMessage(
      chatId,
      "Вибачте щось я розговорився... ось посилання на цей сайт."
    );
...
bot.onText(
  ...
  (msg: { chat: { id: any } }) => {
    ...
    bot.sendMessage(
      chatId,
      `So-Yummy це потужний командний проект з великим функціоналом. Це перший повноцінний проект Владислава виконаний на React та з написанням back-end частини. Ось вам ссиолчка дня ознайлмлення.`
    );

    setTimeout(() => {
      bot.sendMessage(
        chatId,
        "https://irka-bodnariuk.github.io/team-project-so-yummy-frontend/"
      );
...
bot.onText(
  ...
  (msg: { chat: { id: any } }) => {
    ...
    bot.sendMessage(
      chatId,
      `Filmoteka це цікавий командний проект, написаний на Javascript. В цьому пректі Владислав був Тім лідом, що йому дало змогу додати родзинку цьому сайту. поки я вам все не розповів, тримайте ссилку та самі все побчите =)`
    );

    setTimeout(() => {
      bot.sendMessage(
        chatId,
        "https://imvladislav.github.io/team-project-filmoteka/"
      );
    }, 2000); // Таймаут на 4 секунди
  }
);
